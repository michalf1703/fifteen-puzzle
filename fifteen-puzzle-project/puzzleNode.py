...
class PuzzleNode:
    def __init__(self, width, height, puzzle, metric):
        """
        Inicjalizacja węzła dla problemu układanki.

        :param width: szerokość planszy
        :param height: wysokość planszy
        :param puzzle: układanka, lista list
        :param metric: metryka heurystyki (LURD, hamm, manh)
        """
        self.width = width
        self.height = height
        self.board = puzzle
    ...
    def __copy__(self):
        """
        Tworzy nową instancję węzła i kopiuje do niej planszę.
        """
        new_board = copy.deepcopy(self.board)  # tworzymy głęboką kopię planszy
        new_instance = PuzzleNode(self.width, self.height, new_board, self.metric)  # tworzymy nowy węzeł
        ...
        for x in range(0, self.height):
            for y in range(0, self.width):
                ...
                if board_value != 0:
                    ...
                    # obliczenie odległości między pozycją aktualną i docelową
                    distance += abs(proper_x - current_x) + abs(proper_y - current_y)
        return distance

    def hamming_metric(self):
        """
        Funkcja oblicza wartość heurystyki Hamminga dla danego stanu planszy.

        Returns:
        --------
        int
            Wartość heurystyki Hamminga.
        """
